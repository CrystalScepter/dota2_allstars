modifier_item_bottle_060 = class({})

-- Called when the modifier is created
function modifier_item_bottle_060:OnCreated(kv)
	...
	if IsServer() then
		...
		self.bottle_particle =
			ParticleManager:CreateParticle("particles/items_fx/bottle.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetCaster())
	end
end

-- Called when the modifier is destroyed
function modifier_item_bottle_060:OnDestroy()
	if IsServer() then
...
function modifier_item_bottle_060:OnIntervalThink()
...
function modifier_item_bottle_060:IsPurgable()
	return true
end

-- Returns the texture to be displayed in the buff bar in case the item runs out of charges
function modifier_item_bottle_060:GetTexture()
	return "item_bottle"
end
