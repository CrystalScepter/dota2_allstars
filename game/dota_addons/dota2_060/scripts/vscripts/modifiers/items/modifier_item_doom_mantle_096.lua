modifier_item_doom_mantle_096 = class({})

-- Called when the modifier is created
function modifier_item_doom_mantle_096:OnCreated( kv )
...
function modifier_item_doom_mantle_096:DeclareFunctions()
	local funcs = {
...
function modifier_item_doom_mantle_096:GetModifierPreAttack_BonusDamage( params )
	return self.bonus_damage
end

-- Returns the value for the property
function modifier_item_doom_mantle_096:GetModifierPhysicalArmorBonus( params )
	return self.bonus_armor
end

-- Applies the effect upon landing an attack
function modifier_item_doom_mantle_096:OnAttacked( params )
	
	-- Retrieves the amount of doom mantles in the hero's inventory
	local item_count = self:GetCaster():FindAllModifiersByName( "modifier_item_doom_mantle_096" )
...
function modifier_item_doom_mantle_096:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

-- Prevents the modifier from showing up on the buff bar
function modifier_item_doom_mantle_096:IsHidden()
	return true
end

-- Indicates that the item has an aura
function modifier_item_doom_mantle_096:IsAura()
	return true
end

-- Returns the name of the aura modifier
function modifier_item_doom_mantle_096:GetModifierAura()
	return "modifier_item_doom_mantle_096_burn"
end

-- Restricts the aura to only allied units
function modifier_item_doom_mantle_096:GetAuraSearchTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

-- Allows the aura to affect only heroes and creeps (includes summons)
function modifier_item_doom_mantle_096:GetAuraSearchType()
	return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_CREEP
end

-- Returns the radius of the aura
function modifier_item_doom_mantle_096:GetAuraRadius()
	return 0
end
