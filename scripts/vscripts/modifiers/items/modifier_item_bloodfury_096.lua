modifier_item_bloodfury_096 = class({})
...
function modifier_item_bloodfury_096:DeclareFunctions()
	local funcs = {
...
function modifier_item_bloodfury_096:GetModifierPreAttack_BonusDamage( params )
	return self.bonus_damage
end

-- Applies the effect upon landing an attack
function modifier_item_bloodfury_096:OnAttacked( params )
	
	-- Retrieves the amount of bloodfuries in the hero's inventory
	local item_count = self:GetCaster():FindAllModifiersByName( "modifier_item_bloodfury_096" )
	...
	if params.target:IsBuilding() == true then
		return
	end
...
-- Renders the particle effect from lifestealing
function modifier_item_bloodfury_096:PlayEffects( target )
...
function modifier_item_bloodfury_096:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

-- Prevents the modifier from showing up on the buff bar
function modifier_item_bloodfury_096:IsHidden()
	return true
end
